...
class SequenceChecker
{
    ...
    public static void CheckForSubstring(StreamWriter writer, ref bool keepRunning)
    {
        ...
        Console.WriteLine("Writing the next 7 digits and the iteration where it's found to the file.");
        ...
        while (keepRunning)
        {
            try
            {
                ...
                if (index != -1 && index + targetSequence.Length + 7 <= fibString.Length)
                {
                    ...
                    string output = $"Sequence found! Following numbers: {followingDigits}, Iteration: {iteration}";
                    Console.WriteLine(output);
                    writer.WriteLine(output);
                }
